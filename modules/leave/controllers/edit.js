...
//                            $scope.getDuration()
            ...
            $scope.isOK = function isOK(){
                ...
                if($scope.formData.statusId==3||$scope.formData.statusId==2){
                    ...
                    if($scope.formData.leaveTypeId==config.systemArguments.leaveTypeExc){//&&$scope.formData.dataHour != ($scope.formData.payExcT+$scope.formData.noPayExcT)){//调休
                        if(!($scope.formData.payExcT+$scope.formData.noPayExcT)){
                            return false;
                        }
                ...
                if($scope.formData.statusId==1||$scope.formData.statusId==0){
                    if($scope.formData.typeId  == ""||angular.isUndefined($scope.formData.typeId )){
                        return false;
                    }
                    else if($scope.formData.beginTime == ""||angular.isUndefined($scope.formData.beginTime)){
                        return false;
...
//                    $scope.formData.payExcT = $scope.formData.payExcT+$scope.formData.yesterdayExcT
                    $scope.send={
                                ...
                                ,leaveId:$scope.formData.leaveId
                                ,statusId:$scope.formData.statusId
                                ,exchangeFlag:$scope.formData.leaveTypeId==config.systemArguments.leaveTypeExc?1:0//调休
                                ,noPayExcT :$scope.formData.noPayExcT
                                ,payExcT :$scope.formData.payExcT
                                ,yesterdayExcT :$scope.formData.yesterdayExcT

                }
                    Leave.leaveEdit($scope.send).
                        success(function(response){
                            SaveTooltip.showSaveTooltip(response);
                            if(response.status){
